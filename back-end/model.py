...
class Model(object):
    ...
    def load_demo_graph(self, config):
        ...
        # Load variables from the checkpoint
        saver = tf.train.Saver()  # Create a saver for loading variables
        ckpt_path = tf.train.latest_checkpoint(config.CKPT_DIR)  # Locate the latest checkpoint
        if ckpt_path:
            print(f"Loading model from {ckpt_path}...")
            saver.restore(self.sess, ckpt_path)  # Restore weights
            self.warmup(config)
            print("Model loaded successfully.")
        else:
            print("Failed to load model: Checkpoint not found.")
